...
class Categorizer:
    ...
    def guess_best_category(self, memo):
        ...
        mappings_in_code = {
            ...
            "andover water": "water bill",
            " alma ": "healthcare",
            "chewy com": "pet expenses",
            "bbss ": "kid recreation",
            "mahoneys": "home improvement",
            "the home depot": "home improvement",
            "supercuts": "haircut",
            "vzwrlss": "cell phone plan",
            "parking": "parking",
            "eversource": "natural gas",
            "ngrid05": "electricity bill",
            "venmo": "venmo",
            "check": "check",
        }
        ...
        for key in mappings_in_code:
            if key in memo:
                logging.debug(
                    "Using mapped keyword to find category: " + mappings_in_code[key]
                )
    ...
    def get_very_similar_category(self, input_category):
        ...
        for item in self.categories_list:
            distance = levenshtein_distance(
                item["category_name"], input_category, score_cutoff=3
            )
    ...
    def determine_category_id(self, memo):
        ...
        if best_category:
            ...
            if not best_category["category_id"]:
                best_category["category_id"] = self.sqlite_client.get_category_id(
                    category_name
                )
        ...
        if very_similar_category:
            ...
            if very_similar_category_name != best_new_category_name:
                answer = input(
                    "Did you mean: %s? (Y/N): " % very_similar_category_name
                ).lower()
                if answer == "y":
